...
class CartridgeTubeOpenView extends StatefulWidget {
  const CartridgeTubeOpenView({Key? key}) : super(key: key);

  @override
  State<CartridgeTubeOpenView> createState() => _CartridgeTubeOpenViewState();
}

class _CartridgeTubeOpenViewState extends State<CartridgeTubeOpenView> {
  static const desc =
      'Open the wrapped sample tube.\nPeel off the foil on the sample tube.\nInsert the sample tube into the tube stand on the kit as marked.';
  static const videoPath = 'assets/video/step_1.mp4';
  ...
  Widget build(BuildContext context) {
    ...
    final state =
        Provider.of<CleoDevice>(context).state as CartridgeTubeOpenState;
  ...
  void goBackAction() {
    ...
    assert(device.state is CartridgeTubeOpenState);
    (device.state as CartridgeTubeOpenState).goBack();
  }

  void goNextAction() {
    final device = Provider.of<CleoDevice>(context, listen: false);
    assert(device.state is CartridgeTubeOpenState);
    (device.state as CartridgeTubeOpenState).goNext();
  }
}
